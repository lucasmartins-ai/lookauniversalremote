...
pub use messages::*;
pub use sequence::{
    is_valid_sequence_advance, SequenceFilter, SequenceGenerator, SequenceTracker,
};
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidVersion(v) => {
                write!(f, "invalid protocol version: 0x{v:02X} (expected 0x{PROTOCOL_VERSION:02X})")
            }
            Self::UnknownMessageType(t) => {
                write!(f, "unknown message type: 0x{t:02X}")
            }
            Self::InvalidPayloadLength { expected, actual } => {
                write!(f, "invalid payload length: expected {expected} bytes, got {actual}")
            }
