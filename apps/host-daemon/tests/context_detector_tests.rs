...
"#;
    ...
    let (matched_prof, mode) = matcher
        .match_window(&window_steam)
        .expect("Should match steam");
    ...
    let (_, mode_cp) = matcher
        .match_window(&window_cp)
        .expect("Should match Cyberpunk");
    ...
    let (prof_spotify, mode_spotify) = matcher
        .match_window(&window_web_spotify)
        .expect("Should match Spotify regex");
    ...
    let (_, mode_code) = matcher
        .match_window(&window_code)
        .expect("Should match code");
...
fn test_mock_window_detector_mutation() {
    ...
    let initial = detector
        .get_active_window()
        .expect("Get active window succeeds");
    assert_eq!(initial.process_name, "");

    let new_window = ActiveWindowInfo::new(
        "retroarch",
        "RetroArch 1.15",
        Some("retroarch".into()),
        4242,
...
fn test_arbitrator_strict_priority_hierarchy() {
    ...
    let res1 = arbitrator.evaluate(
        Some((&profile_gamepad, TargetControlMode::Gamepad)),
    ...
    let res2 = arbitrator.evaluate(
        Some((&profile_media, TargetControlMode::MediaRemote)),
        TargetControlMode::Trackpad,
    );
